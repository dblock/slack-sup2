    ...
    class Data < SlackRubyBotServer::Events::AppMentions::Mention
      ...
      user_command 'data' do |channel, user, data|
        if channel
          ...
          link = "#{SlackRubyBotServer::Service.url}/api/data?team_id=#{channel.team.id}&channel_id=#{channel.id}&access_token=#{channel.short_lived_token}"
          ...
          link = "#{SlackRubyBotServer::Service.url}/api/data?team_id=#{data.team.id}&access_token=#{data.team.short_lived_token}"
